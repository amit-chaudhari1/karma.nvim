local hsluv = require("karma.hsluv")
...
-- local ns = vim.api.nvim_create_namespace("karma")
...
function util.debug(colors)
    colors = colors or require("karma.colors")
    -- Dump unused colors
...
function util.onColorScheme()
    if vim.g.colors_name ~= "karma" then
        vim.cmd([[autocmd! Karma]])
        vim.cmd([[augroup! Karma]])
    end
end
...
function util.autocmds(config)
    vim.cmd([[augroup Karma]])
    vim.cmd([[  autocmd!]])
    vim.cmd([[  autocmd ColorScheme * lua require("karma.util").onColorScheme()]])
    if config.dev then
        vim.cmd([[  autocmd BufWritePost */lua/karma/** nested colorscheme karma]])
    end
...
function util.terminal(colors)
    ...
    vim.g.terminal_color_14 = colors.red
    ...
    if type(config.colors) == "table" then
        for key, value in pairs(config.colors) do
            ...
            if type(colors[key]) == "table" then
                util.color_overrides(colors[key], {
                    colors = value
                })
            else
